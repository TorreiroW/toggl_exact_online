...
def get_weekly_report(api_key, week_number, year):
    ...
    params = {
        ...
        'workspace_id': '<your_workspace_id>',  # Replace with your Toggl workspace ID
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Download Toggl detailed report for a specific week')
    parser.add_argument('-w', '--week-number', type=int, required=True, help='Week number (e.g., 1-52)')
    parser.add_argument('-y', '--year', type=int, required=True, help='Year')
    parser.add_argument('-k', '--api-key-file', default='toggl_api.dat', help='Toggl API key file')
    ...
    report_data = get_weekly_report(api_key, week_number, year)
    ...
    if report_data:
        ...
        print(f"Report data saved to weekly_report_w{week_number}_y{year}.json")
